...
OperatorPercolationOpeningMatN::OperatorPercolationOpeningMatN()
    :COperator()
{
    ...
    this->setKey("PopulationOperatorPercolationOpeningImageGrid");
    this->setName("percolationOpening");
    this->setInformation("(i,0)=i and A(i,1) = r for maximum radius such that the binary still percole for the coordinate i after the opening with this radius (for a diameter 2*r+1)");
...
void OperatorPercolationOpeningMatN::initState(){
...
void OperatorPercolationOpeningMatN::exec(){
...
COperator * OperatorPercolationOpeningMatN::clone(){
    return new OperatorPercolationOpeningMatN();
}
