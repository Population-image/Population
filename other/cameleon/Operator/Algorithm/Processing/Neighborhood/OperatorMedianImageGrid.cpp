...
OperatorMedianMatN::OperatorMedianMatN()
    :COperator()
{
    this->path().push_back("Algorithm");
    this->path().push_back("Processing");
    this->path().push_back("Neighborhood");
    this->setKey("PopulationOperatorMedianImageGrid");
    this->setName("median");
    this->setInformation("$h(x)=median_{\\forall x'in N(x) }f(x)$ where N(x) is the ball centered in x with radius r and norm n (n=2 by default)\n");
...
void OperatorMedianMatN::initState(){
...
COperator * OperatorMedianMatN::clone(){
    return new OperatorMedianMatN();
}
