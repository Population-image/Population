...
OperatorDilationMatN::OperatorDilationMatN()
    :COperator()
{
    ...
    this->setKey("PopulationOperatorDilationImageGrid");
    this->setName("dilation");
    this->setInformation("$h(x)=max_{\\forall x'in N(x) }f(x)$ where N(x) is the ball centered in x with radius r and norm n (n=2 by default)\n");
...
void OperatorDilationMatN::initState(){
...
void OperatorDilationMatN::exec(){
...
COperator * OperatorDilationMatN::clone(){
    return new OperatorDilationMatN();
}
