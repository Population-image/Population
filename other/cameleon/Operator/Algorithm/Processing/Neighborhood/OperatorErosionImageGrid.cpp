...
OperatorErosionMatN::OperatorErosionMatN()
    :COperator()
{
    this->path().push_back("Algorithm");
    this->path().push_back("Processing");
    this->path().push_back("Neighborhood");
    this->setKey("PopulationOperatorErosionImageGrid");
    this->setName("erosion");
    this->setInformation("$h(x)=min_{\\forall x'in N(x) }f(x)$ where N(x) is the ball centered in x with radius r and norm n (n=2 by default)\n");
...
void OperatorErosionMatN::initState(){
...
COperator * OperatorErosionMatN::clone(){
    return new OperatorErosionMatN();
}
