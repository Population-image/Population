...
OperatorOpeningStructuralMatN::OperatorOpeningStructuralMatN()
    :COperator()
{
        this->path().push_back("Algorithm");
    this->path().push_back("Processing");
    this->path().push_back("Neighborhood");
    this->setKey("PopulationOperatorOpeningStructuralImageGrid");
    this->setName("openingStructuralElement");
    this->setInformation("h(x)=h(x)=Erosion(Dilation(f,N),N) where N(x)={x'+x:strucelt(x'+xcenter) neq 0}\n for instance, for\n(0,1,0)\\\\\n(1,1,1)=structelt, we have  N={(0,0),(1,0),(-1,0),(0,1),(0,-1)}\\\\\n(0,1,0)");
...
void OperatorOpeningStructuralMatN::exec(){
...
COperator * OperatorOpeningStructuralMatN::clone(){
    return new OperatorOpeningStructuralMatN();
}
