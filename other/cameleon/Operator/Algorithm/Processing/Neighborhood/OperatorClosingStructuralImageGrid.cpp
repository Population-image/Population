...
OperatorClosingStructuralMatN::OperatorClosingStructuralMatN()
    :COperator()
{
        this->path().push_back("Algorithm");
    this->path().push_back("Processing");
    this->path().push_back("Neighborhood");
    this->setKey("PopulationOperatorClosingStructuralImageGrid");
    this->setName("closingStructuralElement");
    this->setInformation("h(x)=Closing(f(x),N) where N(x)={x'+x:strucelt(x'+xcenter) neq 0}\n for instance, for\n(0,1,0)\\\\\n(1,1,1)=structelt, we have  N={(0,0),(1,0),(-1,0),(0,1),(0,-1)}\\\\\n(0,1,0)");
...
void OperatorClosingStructuralMatN::exec(){
...
COperator * OperatorClosingStructuralMatN::clone(){
    return new OperatorClosingStructuralMatN();
}
