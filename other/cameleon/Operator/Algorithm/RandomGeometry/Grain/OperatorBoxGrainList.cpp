...
void OperatorBoxGermGrain::exec(){
    ...
    try{
        if(GermGrain2 * phiin = dynamic_cast<GermGrain2 * >(phi.get())){
            ...
            RandomGeometry::box(*phiin,r ,angle);
...
COperator * OperatorBoxGermGrain::clone(){
    return new OperatorBoxGermGrain;
}
