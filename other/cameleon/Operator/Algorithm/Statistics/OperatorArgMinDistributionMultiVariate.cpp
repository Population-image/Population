...
OperatorArgMinDistributionMultiVariate::OperatorArgMinDistributionMultiVariate()
    :COperator()
{
        this->path().push_back("Algorithm");
    this->path().push_back("Statistics");
            this->path().push_back("DistributionMultiVariate");
    this->setKey("PopulationOperatorArgMinDistributionMultiVariate");
    this->setName("argMin");
...
void OperatorArgMinDistributionMultiVariate::initState(){
...
void OperatorArgMinDistributionMultiVariate::exec(){
    ...
    VecF64 value =  Statistics::argMin( f, xmin, xmax, step);
    dynamic_cast<DataPoint *>(this->plugOut()[0]->getData())->setValue(value);
}

COperator * OperatorArgMinDistributionMultiVariate::clone(){
    return new OperatorArgMinDistributionMultiVariate();
}
