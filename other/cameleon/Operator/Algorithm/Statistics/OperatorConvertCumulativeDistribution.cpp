...
OperatorConvertCumulativeDistribution::OperatorConvertCumulativeDistribution()
    :COperator()
{
    ...
    this->setKey("PopulationOperatorConvertCumulativeDistribution");
    this->setName("toCumulativeProbabilityDistribution");
    this->setInformation("ConvertProbabilityDistributio $h(x)=\\int_\\{xmin\\}^\\{x\\}dx |f(x)|/norm$ for xmin<=x<xmax, 0 otherwise with norm =$\\int_\\{xmin\\}^\\{xmax\\} |f(x)|dx$");
...
void OperatorConvertCumulativeDistribution::initState(){
...
void OperatorConvertCumulativeDistribution::exec(){
    ...
    Distribution dist= Distribution(Statistics::toCumulativeProbabilityDistribution(f,xmin,xmax,step));

    dynamic_cast<DataDistribution *>(this->plugOut()[0]->getData())->setValue(dist);

}

COperator * OperatorConvertCumulativeDistribution::clone(){
    return new OperatorConvertCumulativeDistribution();
}
