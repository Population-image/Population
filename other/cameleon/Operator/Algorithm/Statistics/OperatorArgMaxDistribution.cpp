...
OperatorArgMaxDistribution::OperatorArgMaxDistribution()
    :COperator()
{
        this->path().push_back("Algorithm");
    this->path().push_back("Statistics");
    this->path().push_back("Distribution");
    this->setKey("PopulationOperatorArgMaxDistribution");
    this->setName("argMax");
...
void OperatorArgMaxDistribution::initState(){
...
void OperatorArgMaxDistribution::exec(){
    ...
    double value =  Statistics::argMax( f, xmin, xmax, step);
    dynamic_cast<DataNumber *>(this->plugOut()[0]->getData())->setValue(value);
}

COperator * OperatorArgMaxDistribution::clone(){
    return new OperatorArgMaxDistribution();
}
