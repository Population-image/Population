...
OperatorDiffMatN::OperatorDiffMatN()
    :COperator()
{
    ...
    this->setKey("PopulationOperatorDiffImageGrid");
    this->setName("XOR");
    this->setInformation("h(x)=255 for f1(x) neq f2(x), 0 otherwise\n");
...
void OperatorDiffMatN::exec(){
    ...
    BaseMatN * fc1= f1.get();
    BaseMatN * fc2= f2.get();
    try{Dynamic2Static<TListImgGridScalarAndRGB>::Switch(func,fc1,fc2,h,Loki::Type2Type<MatN<2,int> >());}
...
COperator * OperatorDiffMatN::clone(){
    return new OperatorDiffMatN();
}
