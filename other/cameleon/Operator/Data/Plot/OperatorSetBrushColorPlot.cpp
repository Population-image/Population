...
OperatorSetBrushColorPlot::OperatorSetBrushColorPlot(){
        this->path().push_back("Data");
    this->path().push_back("Plot");
    this->setKey("OperatorSetBrushColorPlot");
    this->setName("brushColor");
    this->setInformation("set the Brush (color under the plot) of the graph where red coresponds to (r=255,g=0,b=0), green (r=0,g=255,b=0) and blue (r=0,g=0,b=255) and any color a ratio of these three colors");
...
void OperatorSetBrushColorPlot::initState(){
...
void OperatorSetBrushColorPlot::exec(){
    ...
    graph->VGraph()[0].setBrush(RGBUI8(r,g,b));

    dynamic_cast<DataPlot *>(this->plugOut()[0]->getData())->setData(graph);
}
COperator * OperatorSetBrushColorPlot::clone(){
    return new OperatorSetBrushColorPlot();
}
