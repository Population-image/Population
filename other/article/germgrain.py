# -*- coding: utf-8 -*-
"""
Created on Thu Jun 13 09:53:05 2013

@author: tariel
"""
import sys
#sys.path.append("D:/Users/vtariel/Desktop/ANV/Population/bin")#replace by the path where you unzip the downloaded folder
#from populationpythonwin32 import *
sys.path.append("/home/tariel/Population/bin")#replace by the path where you unzip the downloaded folder
from populationpythonlinux import *
import math
def testUniformPoissonPointProcess2D() :
	domain = Vec2F64(512,512)#2d field domain
	lambdafield = 0.001#parameter of the Poisson VecN process
	random = RandomGeometry()
	grain = random.poissonPointProcess(domain,lambdafield)#generate the 2d Poisson VecN process
	d = Distribution(2,"DIRAC")#because the Poisson VecN process has a surface equal to 0, we associate each point with mono-disperse sphere to display the result
	random.sphere(grain,d)
	img = random.continuousToDiscrete(grain)
	img.display()
def testUniformPoissonPointProcess3D() :
	domain = Vec3F64(100,100,100)#2d field domain
	lambdafield = 0.0001 #parameter of the Poisson VecN process
	random = RandomGeometry()
	grain = random.poissonPointProcess(domain,lambdafield)#generate the 2d Poisson VecN process
	d = Distribution(3,"DIRAC")#because the Poisson VecN process has a volume equal to 0, we associate each point with mono-disperse sphere to display the result
	print lambdafield
	random.sphere(grain,d)
	img = random.continuousToDiscrete(grain)
	print lambdafield
...
def testNonUniformPoissonPointProcess2D() :
	img = Mat2UI8("lena.pgm")
	img = Mat2F64(img)
	img=img/255.
	img=img*0.05
	random = RandomGeometry()
	grain = random.poissonPointProcessNonUniform(img)#generate the 2d Poisson VecN process
	d = Distribution(4,"DIRAC")#because the Poisson VecN process has a surface equal to 0, we associate each point with mono-disperse sphere to display the result
	random.sphere(grain,d)
	img = random.continuousToDiscrete(grain)
	img.display()
...
def testSingleSphere() :
	sphere = GrainSphere3()
	size=100
	sphere.x = Vec3F64(size/2,size/2,size/2)
...
def testSingleEllipse() :
	...
	size=100
	ellipse.x = Vec3F64(size/2,size/2,size/2)
	ellipse.setRadius(Vec3F64(size/2,size/4,size/8))
...
def testSingleCylinder() :
	...
	size=100
	cylinder.x = Vec3F64(size/2,size/2,size/2)
...
def testSingleBox() :
	box =  GrainBox3()
	size=50
	box.x = Vec3F64(size/2,size/2,size/2)
	box.radius = Vec3F64(size/8,size/4,size/2)
	germgrain = ModelGermGrain3() 
	germgrain.setDomain(Vec3F64(size,size,size))
...
def testSingleRhombohedron() :
	...
	size=100
	rombohedra.x = Vec3F64(size/2,size/2,size/2)
...
# 	#The following Cartesian coordinates define the four vertices of a tetrahedron with edge-length 2, centered at the origin:
	x1 = Vec3F64( 1, 0, -1./math.sqrt(2.))
	x2= Vec3F64(-1, 0, -1./math.sqrt(2.))
	x3= Vec3F64( 0, 1,  1./math.sqrt(2.))
	x4= Vec3F64( 0,-1,  1./math.sqrt(2.))
...
# 	
	...
	polyhedra.x = Vec3F64(size/2,size/2,size/2);
	germgrain = ModelGermGrain3() 
	germgrain.setDomain(Vec3F64(size,size,size))
