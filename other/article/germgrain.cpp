...
void testUniformPoissonPointProcess2D(){
    Vec2F64 domain(512,512);//2d field domain
...
void testUniformPoissonPointProcess3D(){
    Vec3F64 domain(200,200,200);//2d field domain
...
void testSingleSphere(){
    ...
    double size=100;
    sphere->x = Vec3F64(size/8,size/2,size/2);
...
void testBox(){
    ...
    box->radius = Vec3F64(size/2,size/4,size/8);
    ModelGermGrain3 germgrain;
    germgrain.setDomain(Vec3F64(size,size,size));
...
void testRhombohedra(){
    ...
    double size=100;
    rhombohedra->x = Vec3F64(size/2,size/2,size/2);
...
void testRegularTetrahedron(){
    ...
    Vec3F64 x1( 1, 0, -1./std::sqrt(2.));
    Vec3F64 x2(-1, 0, -1./std::sqrt(2.));
    Vec3F64 x3( 0, 1,  1./std::sqrt(2.));
    Vec3F64 x4( 0,-1,  1./std::sqrt(2.));

    double size=100;

    Vec3F64 normal_x1_x2_x3 = productVectoriel(x1-x2,x1-x3);
    ...
    Vec3F64 normal_x1_x3_x4 = productVectoriel(x1-x3,x1-x4);
    ...
    Vec3F64 normal_x1_x4_x2 = productVectoriel(x1-x4,x1-x2);
    ...
    Vec3F64 normal_x2_x3_x4 = productVectoriel(x2-x3,x2-x4);
...
void testProbilityDistributionNormal(double porosity_volume_fraction=0.6){
        ...
        Vec3F64 domain(256,256,256);//3d field domain
